...
>
				<input type="hidden" name="view" value="search">
					...
					{/if}
				<input type="search" name="keyword" class="field" value="{$searchboxtext}" onfocus="if(this.value == '{$searchboxtext}') {ldelim}this.value = '';{rdelim}" onblur="if (this.value == '') {ldelim}this.value = '{$searchboxtext}';{rdelim}">
				</form>
			</div>
		...
		<div class="ui-grid-a">
			{section name=nr loop=$userlist}				
				<div class="ui-block-a">
					<img src="{$userlist[nr].Avatar}"> 
				</div>
				<div class="ui-block-b">
					<a href="{$URL_user, $userlist[nr].user_login}">{$userlist[nr].user_login}</a>
					<div>{#PLIGG_Visual_User_Profile_Joined#}: {$userlist[nr].user_date}</div>
					<div>{#PLIGG_Visual_User_Profile_Homepage#}: {$userlist[nr].user_url}</div>
					<div>Add/Remove:
						{if $userlist[nr].status eq 0}	
	...
	{else}
		<div class="heading2-inline">{#PLIGG_Visual_Search_NoResults#} '{$search}'</div>
	{/if}
		<br/>
{/if}
...
{if $user_view eq 'setting'}
	...
	<form action="{$my_pligg_base}/user_settings.php?login={$user_username}" target="_self" method="post">
		...
		<div class="user_settings_template">
			...
			{foreach from=$category item=cat name="cate"}
				...
				<div data-role="fieldcontain">
						<input class="custom" type="checkbox" name="chack[]" id="{$cat.category__auto_id}" value="{$cat.category__auto_id}" {if !in_array($cat.category__auto_id,$user_category)} checked="checked"{/if} />
						<label for="{$cat.category__auto_id}">{$cat.category_name}</label>
				</div>				
			{/foreach}
		</div>
...
>
			...
			{/if}
			{if $user_login neq $user_logged_in}
			{if check_for_enabled_module('simple_messaging',0.6) && $is_friend}{if $friends}
				<div class="padding-bottom"><img src="{$my_pligg_base}/modules/simple_messaging/images/reply.png" border="0" align="absmiddle" /> <a href="{$my_pligg_base}/module.php?module=simple_messaging&view=compose&return={$templatelite.server.REQUEST_URI|urlencode}&to={$user_login}">{#PLIGG_Visual_User_Profile_Message#} <span style="text-transform:capitalize;">{$user_login}</span></a>
				</div>{/if}{/if}
				{if $is_friend gt 0}
				<div class="padding-bottom">
					<img src="{$my_pligg_base}/templates/{$the_template}/images/user_delete.gif" align="absmiddle" />
	...
	<div id="wrapper">
		...
		<h3>{#PLIGG_Visual_User_PersonalData#}</h3>
			...
			<table style="border:none" width="100%">
				...
				<tr>
					<td colspan="2"><input type="button" value="{#PLIGG_Visual_User_Profile_Modify#}" onclick="location='{$URL_Profile}'"></td>
				</tr>
				{/if}
		...
		<div data-role="collapsible">
		<h3>{#PLIGG_Visual_User_Profile_User_Stats#}</h3>
			<fieldset>
		...
		<div data-role="collapsible">
		<h3>{#PLIGG_Visual_User_Profile_User_Groups#}</h3>
			<fieldset>
		...
		{if $Allow_Friends neq "0"}
		<div data-role="collapsible">
		<h3>{#PLIGG_Visual_User_Profile_Friends#}</h3>
			<fieldset>
			{if $friends}			
				<table id="friendlist">
					<thead>
