...
{checkActionsTpl location="tpl_pligg_profile_info_start"}
	...
	{if $UseAvatars neq false}
			...
			<br />
			<input type="radio" id="avatarsource1" name="avatarsource" value="" {if $user_avatar_source eq ""}checked="checked"{/if} /> 
			<label for="avatarsource1">{#PLIGG_Visual_Profile_UseDefaultAvatar#}</label>
			<input type="radio" id="avatarsource2" name="avatarsource" value="useruploaded" {if $user_avatar_source eq "useruploaded"}checked="checked"{/if}> 
			<label for="avatarsource2">{#PLIGG_Visual_Profile_UploadAvatar#}</label>
