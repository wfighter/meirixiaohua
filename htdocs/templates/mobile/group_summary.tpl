<div class="ui-grid-a">
	<div class="ui-block-a">
		<div class="group_avatar">
			<img src="{$imgsrc}" alt="{$group_name} Avatar" class="group_avatar_img" />
		</div>
	</div>
...
{if $is_group_admin eq '1' && $pagename eq 'group_story'}
			<div>
				...
				<span id = "image_upload-1" style="display:none;">
					<fieldset><legend>{#PLIGG_Visual_Profile_UploadAvatar2#}:</legend>
						<form method="POST" enctype="multipart/form-data" name="image_upload_form" action="{$form_action}">
							...
							<button type="submit" value="{#PLIGG_Visual_Profile_AvatarUpload#}" name="action" class="submit">{#PLIGG_Visual_Profile_AvatarUpload#}</button>
						</form> 
					</fieldset>
				</span>
			</div>
