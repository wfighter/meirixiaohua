...
<div class="comment-wrap" {if $comment_status neq "published"}style="background-color: #FFFBE4;border:1px solid #DFDFDF;"{/if}>
	<div class="comment-left"> 	
		...
		<div class="subtext">
			...
			{/if}
                        {if $isadmin}<br />
			<a href="{$my_base_url}{$my_pligg_base}/admin/admin_users.php?mode=view&user={$user_userlogin}">{#PLIGG_Visual_Comment_Manage_User#}</a>
                         {/if} 
		</div>
	</div>
	...
	<div class="comment-right" id="wholecomment{$comment_id}"> 
		{if $comment_votes gte 0} 
			<!--
			<a> 评论编号：{$comment_id} </a>
			{if $comment_parent ne 0}
				<a>  当前回复评论：{$comment_parent} </a>
			{/if}
			-->
			{if $comment_parent ne 0}
				<a>回复评论：</a>
