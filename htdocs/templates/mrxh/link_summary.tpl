...
{if $pagename neq "xstory" && $pagename neq "submit" && $pagename neq "xuser"}
...
{checkActionsTpl location="tpl_pligg_story_start"}
			...
			<div class="vote">
				...
				<span class="subtext" id="xvote-{$link_shakebox_index}">
					{if $link_shakebox_currentuser_votes eq 0 && $link_shakebox_currentuser_reports eq 0}
						<div><a href="javascript:{$link_shakebox_javascript_vote}"><img src="{$my_pligg_base}/templates/{$the_template}/images/arrow_up.gif" alt="{#PLIGG_Visual_Vote_For_It#}" /></a></div>
						
					{else}
						<div><img src="{$my_pligg_base}/templates/{$the_template}/images/arrow_up_voted.gif" alt="{#PLIGG_Visual_Vote_For_It#}" /></div>
						
					{/if}
					
					<div class="votenumber">
						<span class="votenumber"><a id="xvotes-{$link_shakebox_index}">{$link_shakebox_votes}</a></span>
					</div>
					...
					{if $link_shakebox_currentuser_votes eq 0 && $link_shakebox_currentuser_reports eq 0}
						...
						{if $link_shakebox_currentuser_votes eq 0 && $link_shakebox_currentuser_reports eq 0}
							<div class="bury"><span id="xreport-{$link_shakebox_index}"><span id="linksummaryBury">
								<a href="javascript:{$link_shakebox_javascript_report}"><img src="{$my_pligg_base}/templates/{$the_template}/images/arrow_down.gif" alt="{#PLIGG_Visual_Vote_Bury#}" /></a>
							</span></span></div>
						{/if}
					{else}
						
						<div class="bury"><img src="{$my_pligg_base}/templates/{$the_template}/images/arrow_down_voted.gif" alt="{#PLIGG_Visual_Vote_Bury#}" /></div>
					{/if}
				</span>
			...
			<span class="subtext">
				...
				{if $link_group_id neq 0}
          		<span id="groupstorystatus">
				    <a href ="{$my_base_url}{$my_pligg_base}/group_story.php?id={$link_group_id}">{$link_group_name}</a>
	...
	{if $pagename eq "story"}{checkActionsTpl location="tpl_pligg_story_body_start_full"}{else}{checkActionsTpl location="tpl_pligg_story_body_start"}{/if}{if $pagename eq "story" or $link_shakebox_index eq 1}{php}global $the_template;include("templates/".$the_template."/js/jquery/jdeco.php");{/php}{/if}
	...
	{if $viewtype neq "short"}
		<span class="news-body-text">
			...
			{if $pagename neq "story" && $pagename neq "submit"} {if $content_truncate eq "true"}<br /><br /><p><a href="{$story_url}"><u><font >……{#PLIGG_Visual_Read_More#}……</font></u></a></p>{/if}{/if}
      </span>
		</span>
