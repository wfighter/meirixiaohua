...
function get_comments ($fetch = false){
	...
	if ($comments) {
		...
		if (isMobile == 'true') $output .= '<ul data-role="listview">';
		...
		foreach($comments as $dbcomment) {
			...
			if (isMobile == 'true') $output .= '<li data-role="list-divider">'.$link->title.'</li>';
			...
			if ($comments2) {
				if (isMobile != 'true') $output .= '<div class="child-comment">';				
				require_once(mnminclude.'comment.php');
				...
				if (isMobile != 'true') $output .= "</div>\n";
			}
 		} 
		
		if (isMobile == 'true') $output .= '</ul><br/>';
