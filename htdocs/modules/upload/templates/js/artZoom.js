	...
	$.fn.artZoom = function(){
		$(this).live('click', function(){
			...
			if (document.body && document.body.scrollTop)
      		{ orgScroll=document.body.scrollTop; }
      		if (document.documentElement && document.documentElement.scrollTop)
      		{ orgScroll=document.documentElement.scrollTop;}

			if ($(this).find('.loading').length == 0) $(this).append('<span class="loading" title="Loading..">Loading..</span>');
			imgTool($(this), maxImg, viewImg,orgScroll);
			return false;
		});
		...
		// 图片工具条
		var imgTool = function(on, maxImg, viewImg,orgScroll) {
			var width = 0,
			height = 0,
			tool = function(){
				...
				box.find('a').bind('click', function(){
					...
					if($(this).hasClass('hideImg') || $(this).hasClass('maxImgLink')) {
						...
						box.prev().show();
           				if (document.body && document.body.scrollTop)
           				{ document.body.scrollTop = orgScroll; }
           				if (document.documentElement && document.documentElement.scrollTop)
           				{ document.documentElement.scrollTop = orgScroll;}
					};
					// 左旋转
