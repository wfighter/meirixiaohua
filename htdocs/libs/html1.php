...
function do_pages($total, $page_size, $thepage, $fetch = false) {
	...
	if ($URLMethod == 1) {
		...
		if (isMobile == 'true') { /* Display pagination for mobile site */
			$output .= '<div class="ui-grid-b">';
			
			$output .= '<div class="ui-block-a">';		
			if($current==1) {			
				$output .= '<a href="#"  data-mini="true" class="ui-disabled" data-role="button" data-icon="arrow-l" data-inline="true" data-iconpos="left">prev</a>';
			} else {
				$i = $current-1;
				if ((pagename == "index" || pagename == "published")  && $i==1)
					$output .= '<a href="'.($query ? '?' : './').$query.'"  data-mini="true" data-role="button" data-icon="arrow-l" data-inline="true" data-iconpos="left">prev</a>';
				else
					$output .= '<a href="?page='.$i.$query.'"  data-mini="true" data-role="button" data-icon="arrow-l" data-inline="true" data-iconpos="left">prev</a>';
			}
			$output .= '</div>';
			...
			if ($total_pages <= 1)
				$output .= '<select id="select-choice-page" data-native-menu="false" data-mini="true" disabled>';
			else
				$output .= '<select id="select-choice-page" data-native-menu="false" data-mini="true">';
				
			for ($i=$start;$i<= $total_pages;$i++) {
				if($i==$current) 
					$output .= '<option selected="selected">'.$i.'</option>';
				elseif ((pagename == "index" || pagename == "published")  && $i==1)
					$output .= '<option value="'.($query ? '?' : './').$query.'">'.$i.'</option>';
				else 
					$output .= '<option value="?page='.$i.$query.'">'.$i.'</option>';
			}
			$output .= '</select>';
			$output .= '</div>';
			
			$output .= '<div class="ui-block-c">';
			if($current<$total_pages) {
				$i = $current+1;
				$output .= '<a href="?page='.$i.$query.'"data-mini="true" class="right" data-role="button" data-icon="arrow-r" data-inline="true" data-iconpos="right">next</a>';
			} else {
				$output .= '<a href="#"  data-mini="true" class="ui-disabled right" data-role="button" data-icon="arrow-r" data-inline="true" data-iconpos="right">next</a>';
			}
			$output .= '</div>';
			$output .= "</div>\n";
		}
