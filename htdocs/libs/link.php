...
class Link {
	...
	function store() {
		...
		$link_group_name = $db->escape($this->link_group_name);
		//$link_group_story_url = $db->escape($this->link_group_story_url);
	...
	function store_basic() {
		...
		$link_group_name = $db->get_var("SELECT group_name FROM `" . table_groups . "` WHERE `group_id` = $this->link_group_id");
		//$link_group_story_url = $db->get_var("SELECT group_story_url FROM `" . table_groups . "` WHERE `group_id` = $this->link_group_id");
...
/////
			...
			$this->link_group_name=$db->get_var("SELECT group_name FROM `" . table_groups . "` WHERE `group_id` = $link->link_group_id");
			//$this->link_group_story_url=$db->get_var("SELECT group_story_url FROM `" . table_groups . "` WHERE `group_id` = $link->link_group_id");
	...
	function read_basic() {
		...
		if(($link = $db->get_row("SELECT link_comments, link_author, link_status, link_randkey, link_category, link_date, link_votes, link_karma, link_published_date,link_group_id FROM " . table_links . " WHERE link_id = $id"))) {
			...
			$this->comments=$link->link_comments;
			$this->link_group_id=$link->link_group_id;
			$this->line_group_name = $db->get_var("SELECT group_name FROM `" . table_groups . "` WHERE `group_id` = $link_group_id");
			$this->read = true;
	...
	function fill_smarty($smarty, $type='full'){
      ...
      if(utf8_strlen($this->content) > StorySummary_ContentTruncate) {
          $smarty->assign('content_truncate', 'true');
      }
		}
		if($type == "full"){
		...
		$smarty->assign('link_group_name', $this->link_group_name);
		//$smarty->assign('link_group_story_url', $this->link_group_story_url);
